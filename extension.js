...
var AppGridButton = GObject.registerClass(
  class AppGridButton extends PanelMenu.Button {
    _init() {
      ...
      this.app_grid_button.connect(
        "button-release-event",
        this._show_apps_page.bind(this)
      );
  ...
  class FavoritesMenu extends PanelMenu.Button {
    ...
    _display_favorites() {
      ...
      for (let fav_index = 0; fav_index < this.list_fav.length; ++fav_index) {
        ...
        this.item = new PopupMenu.PopupImageMenuItem(
          this.fav.get_name(),
          this.fav_icon.get_gicon()
        );
        ...
        this.item._draggable = DND.makeDraggable(this.item, {
          dragActorOpacity: HIDDEN_OPACITY,
        });

        this.item._draggable.connect("drag-end", this._on_drag_end.bind(this));
        this.item._draggable.connect(
          "drag-cancelled",
          this._on_drag_end.bind(this)
        );
      }
    }
...
var WorkspacesBar = GObject.registerClass(
  class WorkspacesBar extends PanelMenu.Button {
    _init() {
      ...
      this._windows_changed = this.window_tracker.connect(
  ...
  class WorkspaceButton extends St.Bin {
    ...
    acceptDrop(source) {
      ...
      if (source.is_babar_favorite) {
        WM.get_workspace_by_index(this.number).activate(
          global.get_current_time()
        );
        AppFavorites.getAppFavorites()
          .getFavorites()
          [source.fav_index].open_new_window(-1);
      }
...
var WindowButton = GObject.registerClass(
  class WindowButton extends St.Bin {
    ...
    acceptDrop(source) {
      // favorite menu item
      if (source.is_babar_favorite) {
        WM.get_workspace_by_index(this.workspace_number).activate(
          global.get_current_time()
        );
        AppFavorites.getAppFavorites()
          .getFavorites()
      ...
      // window button
      if (
        source.is_babar_task &&
        source.workspace_number !== this.workspace_number
      ) {
...
class Extension {
  ...
  _show_places_icon(show_icon) {
    ...
    if (this.places_indicator) {
      this.places_indicator.remove_child(
        this.places_indicator.get_first_child()
      );
      if (show_icon) {
